...
export const useQuarterlyData = () => {
  ...
  const fetchQuarterlyData = async (): Promise<void> => {
    loading.value = true
    error.value = null

    try {
      const data = await apiRequest<QuarterlyResponse>('/api/quarterly/me')
      rawData.value = data
    } catch (err: any) {
  ...
  const getQuarterPercentages = (year: number): QuarterPercentage[] | null => {
    ...
    const yearData = rawData.value.data.find(d => d.year === year)
    if (!yearData) return null
    return yearData.quarters.map(q => ({ quarter: q.quarter, percentage: q.percentage }))
  }
